        ...
        private static IReadOnlyDictionary<string, HelpTopic> BuildTopics()
        {
            var topics = new Dictionary<string, HelpTopic>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                ["App.StoreUpdate"] = Topic(
                    ...
                    "Use it when PS7 ScriptDesk shows an automatic update notification or when you choose Help > Check for Updates.",
                    ...
                    Section("Automatic startup checks", false,
                        ...
                        "Startup stays quiet for no-update results, unpackaged or local builds, sideloaded or test packages, and routine manual Store instructions."),
                    Section("Manual checks", false,
                        "Help > Check for Updates runs the same safe update detection service on demand and opens this window with the result.",
                        "For Microsoft Store-managed installations, the result can be an available update, no available update, manual Store instructions, or an unavailable check.",
                        "For unpackaged/local builds and sideloaded or test packages, Store-managed installation is not available; the window explains the current build state and manual options instead."),
                ...
                ["Menu.File"] = Topic(
                    ...
                    Section("Typical uses", false,
                        ...
                        "Close the current tab or close every open tab.",
                ...
                ["Command.CloseTab"] = Topic(
                    ...
                    related: new[] { "Command.CloseAll", "Editor.TabStrip", "Command.Save" }),

                ["Command.CloseAll"] = Topic(
                    "Command.CloseAll",
                    "Close All",
                    "Closes every open editor tab after handling unsaved changes one tab at a time.",
                    "Use it when you want to clear the current workspace without exiting PS7 ScriptDesk.",
                    "If any tab has unsaved changes, the app asks whether to save, discard, or cancel. Cancel stops the Close All operation and leaves the remaining tabs open.",
                    Section("What to expect", false,
                        "Saved and unchanged tabs close immediately.",
                        "Unsaved tabs prompt before they close.",
                        "If all tabs close successfully, the app opens a fresh untitled tab so the editor stays usable."),
                    Section("Shortcut", false, "Ctrl+Shift+W"),
                    related: new[] { "Command.CloseTab", "Command.Save", "Editor.TabStrip" }),

                ["Menu.RecentScripts"] = Topic(
                    "Menu.RecentScripts",
                    "Recent Scripts",
                    "Shows recently opened script files in the File menu.",
                    "Use it when you want to reopen a script without browsing for it again.",
                    "This list tracks file paths the app has opened before. If a file was moved or deleted, opening that entry can fail and the status/output areas report the problem.",
                    related: new[] { "Menu.File", "Command.OpenFile" }),

                ["Menu.Tools"] = Topic(
                    "Menu.Tools",
                    "Tools menu",
                    "The Tools menu holds diagnostics and support actions instead of day-to-day editing commands.",
                    "Use it when you need logs, packaged support output, or developer-diagnostics controls.",
                    "Most normal scripting work does not require this menu, but it is useful when you are troubleshooting or preparing a bug report.",
                    related: new[] { "Help.Logs", "Help.Troubleshooting" }),
                ...
                ["Command.RunSelection"] = Topic(
                    ...
                    "Run Selection",
                    "Runs the current editor selection in the shared terminal session. If nothing is selected, it runs the current line.",
                    "Use it for quick experiments, one-off commands, or executing one part of a script without running the whole file.",
                    "Run Selection executes in the current session scope. The code can depend on variables, functions, modules, and location changes already present in that shared session.",
                    Section("Shortcut", false, "F8"),
                    Section("Workflow", true,
                        "Highlight the code you want to run, or place the caret on the line you want to run.",
                        ...
                        "If no text is selected, PS7 ScriptDesk falls back to the current line to match the ISE-style F8 workflow."),
                    related: new[] { "Command.RunScript", "Console.Area", "Editor.Surface", "Help.Troubleshooting" }),

                ["Command.Interrupt"] = Topic(
                    "Command.Interrupt",
                    "Interrupt",
                    "Tries to stop the current command or script and return control to the console.",
                    "Use it when a run hangs, takes too long, or you started the wrong command.",
                    "Interrupt first sends the normal Ctrl+C-style stop path into the live terminal session. If the running PowerShell session does not respond, PS7 ScriptDesk can restart only the PowerShell session it owns and then reopen a fresh prompt automatically.",
                    Section("Best use", false,
                        "Long loops",
                        "Accidental full-script runs",
                        "Commands waiting for input or taking too long",
                        "Recovering from a stuck interactive session without closing the whole app"),
                    Section("What to expect", false,
                ...
                ["Help.Logs"] = Topic(
                    "Help.Logs",
                    "Logs and support output",
                    "PS7 ScriptDesk can open its diagnostic log folder, copy that path, or package support logs into a ZIP file.",
                    "Use these commands when you are troubleshooting or preparing a bug report.",
                    $"Standard app logs are stored under {AppLogger.CurrentLogDirectory}. Developer-diagnostics logs are stored separately when that optional feature is enabled.",
                    Section("Useful support details", false,
                        "Include the app version and PowerShell version.",
                        "Describe what you clicked and what happened.",
                        "Attach a short script snippet if it is safe to share.",
                        "Include screenshots and the packaged logs when possible."),
                    related: new[] { "Menu.Tools", "Help.Troubleshooting", "Help.About" }),
                ...
                ["Runtime.Area"] = Topic(
                    ...
                    "The app requires a validated PowerShell 7.x runtime and only treats `pwsh.exe` from PowerShell Core 7.0 or newer as usable.",
                ...
                ["Runtime.Path"] = Topic(
                    ...
                    "Use it when you want to verify exactly which `pwsh.exe` executable the shell is targeting.",
                ...
                ["Editor.SyntaxPanel"] = Topic(
                    ...
                    "This pane is for editor diagnostics, not for live PowerShell runtime output. Runtime errors from a script still appear in the Console.",
                    Section("How to read it", false,
                        "Use the Problems tab at the bottom of the window to switch from Console output to editor diagnostics.",
                        ...
                        "Fix the script and watch the pane update after the background parse completes."),
                    related: new[] { "Console.Area", "Editor.Footer", "Editor.Surface", "Help.Troubleshooting" }),
                ...
                ["Console.Area"] = Topic(
                    ...
                    Section("Main parts", false,
                        ...
                        "Console and Problems bottom tabs",
                        "Session status text",
                ...
                ["Editor.Metadata"] = Topic(
                    ...
                    Section("Where metadata shows up", false,
                        "Ctrl+Space for completion suggestions in the editor",
                        "F1 quick info in the editor when the caret is on a known command or parameter",
                        "The metadata status badge in the shell footer while warmup or refresh is still running"),
                ...
                ["Help.Troubleshooting"] = Topic(
                    ...
                    Section("Script appears not to run or Run is disabled", true,
                        ...
                        "If nothing is selected, Run Selection falls back to the current line instead of doing nothing."),
                    ...
                    Section("Logs and bug reports", false,
                ...
                ["Help.Packaging"] = Topic(
                    ...
                    "Store/MSIX builds can surface Microsoft Store update guidance at startup when an update needs attention, and Help > Check for Updates can show the current update-check result on demand.",
                    Section("Current runtime-facing behavior", false,
                        ...
                        "Unpackaged/local builds and sideloaded or test packages cannot install through the Store update service; manual checks explain that state instead of starting Store installation.",
