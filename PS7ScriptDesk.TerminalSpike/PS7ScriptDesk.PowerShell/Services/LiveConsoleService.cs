...
namespace PS7ScriptDesk.PowerShell.Services
{
    ...
    ///
    /// The exec-done sentinel (<c>##PSSTUDIO_EXEC_DONE##</c>) is stripped before
    /// any data reaches xterm.js so it never appears in the visible terminal.
    /// ──────────────────────────────────────────────────────────────────────────
    /// </summary>
    public class LiveConsoleService : ILiveConsoleService
    {
        ...
        // Sentinel written to the terminal after every script dispatch.
        // When it appears in terminal output, the script has finished executing.
        // The string is stripped from visible output before publishing to the UI.
        private const string ExecDoneToken = "##PSSTUDIO_EXEC_DONE##";
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await WriteTerminalInputAsync(commandText + Environment.NewLine, cancellationToken).ConfigureAwait(false);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!TryBeginCommandDispatch(isScript: true, snapshotPath: snapshotPath, out var dispatchFailure))
            {
                throw new InvalidOperationException(dispatchFailure ?? "Another terminal operation is already running.");
            }

            var startedAt = DateTime.Now;
            var invocationLine = BuildScriptInvocationLine(snapshotPath, executeInCurrentScope);
            var completionLine = BuildCompletionMarkerLine();
            var scriptCommand = string.Join(Environment.NewLine, invocationLine, completionLine) + Environment.NewLine;
            RegisterHiddenOutputFragment(invocationLine);
            RegisterHiddenOutputFragment(completionLine);
            AppLogger.Info(
                "LiveConsole",
                $"Sending visible editor script command to the live ConPTY terminal. SnapshotPath={snapshotPath}, ScriptLength={scriptContent?.Length ?? 0}, ExecuteInCurrentScope={executeInCurrentScope}.");
        ...
        private static string BuildScriptInvocationLine(string snapshotPath, bool executeInCurrentScope)
        {
            var invocationOperator = executeInCurrentScope ? "." : "&";
            return invocationOperator + " " + QuotePowerShellSingleQuotedString(snapshotPath);
        }

        private static string BuildCompletionMarkerLine()
        {
            return "Write-Host " + QuotePowerShellSingleQuotedString(ExecDoneToken);
        }
        ...
        private void StartRedirectedSession(PowerShellRuntimeInfo runtime, string workingDirectory, Action<ExecutionOutputRecord> onOutput)
        {
            var process = new Process
            {
                StartInfo = new ProcessStartInfo
                {
                    ...
                    Arguments = "-NoLogo -NoExit -ExecutionPolicy Bypass -Command " + QuoteCommandArgument("try { Set-PSReadLineOption -PredictionSource None -ErrorAction SilentlyContinue } catch { }"),
        ...
        private void CancelPendingCommandDispatch(bool deleteSnapshot)
        {
            ...
            lock (_syncRoot)
            {
                ...
                {
                    snapshotPath = _pendingSnapshotPaths.Dequeue();
                }
            ...
            if (deleteSnapshot && !string.IsNullOrWhiteSpace(snapshotPath))
            {
                TryDeleteSnapshot(snapshotPath);
            }
        }
        ...
        private string FilterInternalTerminalOutput(string raw, out bool hasSentinel)
        {
            ...
            lock (_syncRoot)
            {
                ...
                {
                    if (raw.Contains(ExecDoneToken, StringComparison.Ordinal))
                    {
                        hasSentinel = true;
                        return raw.Replace(ExecDoneToken, string.Empty, StringComparison.Ordinal);
                    }
                ...
                while (TryReadTerminalSegment(ref _hiddenOutputBuffer, out var segment))
                {
                    ...
                    {
                        if (sanitizedSegment.Contains(ExecDoneToken, StringComparison.Ordinal) ||
                            sanitizedSegment.Contains("##PSSTUDIO_EXEC", StringComparison.OrdinalIgnoreCase))
                        {
                            hasSentinel = true;
                        }
                    ...
                    if (segment.Contains(ExecDoneToken, StringComparison.Ordinal))
                    {
                        hasSentinel = true;
                        segment = segment.Replace(ExecDoneToken, string.Empty, StringComparison.Ordinal);
                    }
                ...
                if (_hiddenOutputBuffer.Length > 0)
                {
                    ...
                    {
                        if (_hiddenOutputBuffer.Contains(ExecDoneToken, StringComparison.Ordinal))
                        {
                            hasSentinel = true;
                            var remainder = _hiddenOutputBuffer.Replace(ExecDoneToken, string.Empty, StringComparison.Ordinal);
                    ...
                    {
                        var safeLength = Math.Max(0, _hiddenOutputBuffer.Length - 128);
                        if (safeLength > 0)
                        {
                            var safePart = _hiddenOutputBuffer[..safeLength];
                            if (safePart.Contains(ExecDoneToken, StringComparison.Ordinal))
                            {
                                hasSentinel = true;
                                safePart = safePart.Replace(ExecDoneToken, string.Empty, StringComparison.Ordinal);
                            }

                            if (!IsInternalExecutionEcho(RemoveControlSequences(safePart)))
                            {
                                filtered.Append(safePart);
                            }

                            keepRemainder.Append(_hiddenOutputBuffer[safeLength..]);
                        }
                        else
                        {
                            keepRemainder.Append(_hiddenOutputBuffer);
        ...
        private static bool IsInternalExecutionEcho(string sanitizedSegment)
        {
            ...
            return normalized.Contains("Write-Host '##PSSTUDIO_EXEC", StringComparison.OrdinalIgnoreCase) ||
                   normalized.Contains("Write-Host \"##PSSTUDIO_EXEC", StringComparison.OrdinalIgnoreCase) ||
                   normalized.Contains("##PSSTUDIO_EXEC", StringComparison.OrdinalIgnoreCase) ||
        ...
        private static bool IsPotentialInternalExecutionEchoPrefix(string sanitizedSegment)
        {
            ...
            return normalized.Contains("Write-Host '##PSSTUDIO", StringComparison.OrdinalIgnoreCase) ||
                   normalized.Contains("Write-Host \"##PSSTUDIO", StringComparison.OrdinalIgnoreCase) ||
                   normalized.Contains("##PSSTUDIO", StringComparison.OrdinalIgnoreCase) ||
                   normalized.Contains("Write-Host ''", StringComparison.OrdinalIgnoreCase) ||
                   normalized.Contains("Write-Host \"\"", StringComparison.OrdinalIgnoreCase) ||
                   normalized.EndsWith("Write-Host '", StringComparison.OrdinalIgnoreCase) ||
                   normalized.EndsWith("Write-Host \"", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static string CreateExecutionSnapshot(string documentDisplayName, string scriptContent)
        {
            ...
            var baseName = Path.GetFileNameWithoutExtension(documentDisplayName);
            ...
            var fileName = $"{TerminalSnapshotFilePrefix}{DateTime.UtcNow:yyyyMMdd_HHmmss_fff}_{Environment.ProcessId}_{Guid.NewGuid():N}_{baseName}.ps1";
        ...
        {
            return Path.Combine(Path.GetTempPath(), "PS7ScriptDesk", "TerminalSnapshots");
        }
